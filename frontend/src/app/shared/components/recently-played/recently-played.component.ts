        ...
        >
          <div class="relative">
            ...
            <div class="play-overlay">
              <button class="bg-spotify-green text-black rounded-full w-12 h-12 flex items-center justify-center">
                ▶️
              </button>
            </div>
          </div>
          <div class="mt-3">
            <h3 class="theme-text font-medium truncate">{{ item.song.title }}</h3>
            <p class="theme-muted text-sm truncate">{{ item.song.artist }}</p>
            <p class="theme-muted text-xs mt-1">{{ getTimeAgo(item.playedAt) }}</p>
          </div>
        </div>
      </div>
    </div>
